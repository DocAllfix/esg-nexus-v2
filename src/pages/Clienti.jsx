...
export default function Clienti() {
  ...
  const { data: clienti = [], isLoading, error, create } = useClienti();
  ...
  return (
    <DataGuard data={clienti} isLoading={isLoading} error={error}>
      <div>
        <PageHeader
          title="Clienti"
              ...
              {filtered.map(cliente => (
                ...
                >
                  ...
                  <td className="px-5 py-3 text-muted-foreground">
                    {cliente.fatturato_eur != null ? `€${(cliente.fatturato_eur / 1_000_000).toFixed(1)}M` : "—"}
                  </td>
                  <td className="px-5 py-3">
