...
function getFaseStato(eng, faseCode) {
  const fase = eng.fasi?.find(f => f.code === faseCode);
  if (!fase) return { stato: "non_iniziata", progress: 0 };
  return { stato: fase.stato, progress: fase.progress ?? 0 };
}

function FaseCell({ stato, progress, onClick }) {
    ...
    >
      ...
      {isInCorso && (
        <div className="flex flex-col items-center gap-0.5">
          <span className="text-[10px] font-bold text-primary tabular-nums leading-none">{progress}%</span>
...
export default function StatoAvanzamento() {
  ...
  // KPI calcolati dai mock reali
  const totFasi = engagements.length * FASI.length;
  const fasiFilled = engagements.reduce((acc, eng) => acc + (eng.fasi?.filter(f => f.stato === "completata").length ?? 0), 0);
  const fasiInCorso = engagements.reduce((acc, eng) => acc + (eng.fasi?.filter(f => f.stato === "in_corso").length ?? 0), 0);
  const pctGlobale = totFasi > 0 ? Math.round((fasiFilled / totFasi) * 100) : 0;

  const engFiltered = engagements.filter(eng => {
    const q = search.toLowerCase();
    return !q || eng.cliente_nome?.toLowerCase().includes(q) || eng.project_code?.toLowerCase().includes(q);
  });
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-1.5 mb-0.5">
                                <span className="font-mono text-[10px] bg-muted px-1.5 py-0.5 rounded text-muted-foreground">
                                  {eng.project_code?.split("-").slice(0, 3).join("-")}
                                </span>
                                <StatusBadge status={eng.stato} />
                              </div>
                              <p className="font-semibold text-xs truncate max-w-[160px]">{eng.cliente_nome}</p>
                              <p className="text-muted-foreground text-[10px]">{eng.anno} · {eng.responsabile?.split(" ").slice(-1)[0]}</p>
                            </div>
