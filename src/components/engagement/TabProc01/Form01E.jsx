...
export default function Form01E({ engagementId }) {
  const { data: d, status, updateField, updateStatus, saveForm, isSaving } = useFormData(engagementId, "01E");

  const rischi = d?.rischi ?? [];
  const setRischio = (i, k, v) => {
    const n = [...rischi];
    n[i] = { ...n[i], [k]: ["prob", "impatto", "score"].includes(k) ? (Number(v) || 0) : v };
    updateField("rischi", n);
  };

  const rischiCalcolati = rischi.map(r => ({ ...r, score: (r.prob || 0) * (r.impatto || 0) }));
      ...
      <FormSection title="Registro rischi" cols={1}>
        <div className="space-y-3">
          {rischiCalcolati.map((r, i) => {
            ...
            return (
              <div key={r.id || i} className={cn("rounded-lg border p-4 space-y-3", cfg.border)}>
                ...
                <div className="flex gap-3 flex-wrap">
                  ...
                  <select value={r.stato || "aperto"} onChange={e => setRischio(i, "stato", e.target.value)} className="ml-auto text-xs px-2 py-0.5 rounded border border-border bg-background">
