...
export default function Form02D({ engagementId }) {
  const { data: d, status, updateField, updateStatus, saveForm, isSaving } = useFormData(engagementId, "02D");

  const mapping = d?.mapping ?? [];
  const setRow = (i, k, v) => {
    const n = [...mapping];
    n[i] = { ...n[i], [k]: v };
    updateField("mapping", n);
  };
  ...
  return (
    <FormWrapper
      ...
      ruleBox="📋 Per ogni tema materiale definire: standard di riferimento, KPI previsti, referente interno, capitolo del bilancio. Documentare le esclusioni."
      ruleBoxType="info"
      status={status}
    ...
    >
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Temi materiali totali", value: mapping.length, color: "text-foreground" },
          { label: "Inclusi nel bilancio", value: inclusi, color: "text-green-700" },
          { label: "Esclusi / ridotti", value: mapping.length - inclusi, color: "text-amber-600" },
        ].map(k => (
      ...
      <FormSection title="Mapping temi materiali → bilancio" cols={1}>
        <div className="space-y-3">
          {mapping.map((m, i) => (
            <div key={m.id || i} className={cn("rounded-lg border p-4 space-y-3", m.incluso ? "border-border" : "border-gray-200 bg-gray-50/50 opacity-70")}>
      ...
      <FormSection title="Nota metodologica" cols={1}>
        <Field label="Criteri di inclusione/esclusione e nota al perimetro">
          <Textarea value={d?.nota_metodologica} onChange={v => updateField("nota_metodologica", v)} rows={4} />
        </Field>
      </FormSection>
    </FormWrapper>
  );
