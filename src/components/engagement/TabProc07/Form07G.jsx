...
const CHECKLIST_CHIUSURA = [
  ...
  { id: 7,  voce: "Documentazione archiviata con accessi corretti",              proc: "PROC-07", critico: true },
...
export default function Form07G({ engagementId }) {
  const { data: d, status, updateField, updateStatus, saveForm, isSaving } = useFormData(engagementId, "07G");

  const checks = d?.checks ?? CHECKLIST_CHIUSURA.map(c => ({ ...c, ok: false }));
  ...
  const chiusoFormalmente = d?.chiuso_formalmente ?? false;
  ...
  return (
    <FormWrapper
      ...
      meta={{ "Data chiusura": d?.data_chiusura || "—", "Chiuso da": d?.chiuso_da || "—" }}
      ruleBox={tutteOk ? "Tutti i prerequisiti soddisfatti — Engagement pronto per la chiusura formale." : "Completare le attività obbligatorie prima di chiudere formalmente l'engagement."}
    ...
    >
      {chiusoFormalmente ? (
        <div className="rounded-2xl border-2 border-green-400 bg-green-50 p-8 text-center space-y-3">
          ...
          <p className="text-sm text-green-600">Chiuso il {d?.data_chiusura || "—"} da {d?.chiuso_da || "—"}</p>
      ...
      <FormSection title="Dati di chiusura formale">
        ...
        <Field label="Chiuso da (Partner responsabile)">
          <Input value={d?.chiuso_da} onChange={v => updateField("chiuso_da", v)} />
        </Field>
      ...
      {!chiusoFormalmente && (
        <div className="pt-2">
          <button
            disabled={!tutteOk}
            onClick={() => updateField("chiuso_formalmente", true)}
